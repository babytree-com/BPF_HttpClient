<?php
namespace BPF\HttpClient\Socket;

use BPF\HttpClient\Psr\Response;
...
class HttpSocket {
    ...
    public function setDebug($debug_stream = null) {
        $this->debug = true;
        $this->debug_stream = \BPF\HttpClient\debug_resource($debug_stream);
    }
    ...
    public function connect() {
        ...
        if ($this->use_proxy && $this->use_ssl) {
            $message = [
                sprintf('%s %s:%s %s', 'CONNECT', $this->request->getUri()->getHost(), $this->request->getUri()->getPort(true), \BPF\HttpClient\getHttpVersion($this->request->getProtocolVersion())),
            ...
            \BPF\HttpClient\parseResponseMessage($message, $response, $is_finish);
